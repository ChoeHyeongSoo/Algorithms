...
class Solution {
    public int solution(int[][] info, int n, int m) {
        ...
        return ans < n ? ans : -1; // INF 가지고 돌아오면 불가능
    }

    static final int INF = 100000;
    static int[][] INFO, table;

    public int dp(int i, int b) {
        ...
        if (table[i+1][b]!=-1 && table[i+1][b]!=INF) return table[i+1][b];

        int result = dp(i-1, b-INFO[i][1]);

            result = Math.min(result, dp(i-1, b) + INFO[i][0]);
