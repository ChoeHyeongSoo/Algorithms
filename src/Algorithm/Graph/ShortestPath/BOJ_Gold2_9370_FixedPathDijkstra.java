...
public class BOJ_Gold2_9370_FixedPathDijkstra{
    public static void main(String[] args) throws Exception {
        ...
        for (int tc = 0; tc < T; tc++) {
            ...
            for (int i = 0; i < m; i++) {
                ...
                adj[curr].add(new Node_9370(next, weight)); adj[next].add(new Node_9370(curr, weight));
            }
    ...
    static int n, s, g, h;
    static List<Node_9370>[] adj;


    public static int[] get_cost(int s) {
        PriorityQueue<Node_9370> pq = new PriorityQueue<>(); // g,h 방문 안 한 상태로 후보지 들어오면 컨티뉴
        int[] costs = new int[n + 1];
        Arrays.fill(costs, Integer.MAX_VALUE);
        pq.add(new Node_9370(s, 0));
        costs[s] = 0;
        int cnt = 0;
        while (!pq.isEmpty()) {     // 다익스트라 : 목적지가 아닌, 각 노드로 가는 모든 최단 거리 정보를 가진 배열 반환
            Node_9370 curr = pq.poll();
            if (cnt==n) {
                return costs;
            }
            for (Node_9370 next : adj[curr.idx]) {
                if (costs[curr.idx]+ next.d < costs[next.idx]) {
                    ...
                    pq.add(new Node_9370(next.idx, costs[next.idx]));
                }
            }
        }
        return costs;
...
class Node_9370 implements Comparable<Node_9370>{
    ...
    public Node_9370(int idx, int d) {
        this.idx = idx;
        this.d = d;
    }
