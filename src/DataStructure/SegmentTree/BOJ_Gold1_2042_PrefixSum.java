...
public class BOJ_Gold1_2042_PrefixSum {
    ...
    static SegmentTree prefix_sum_TopDown;
    static SegmentTree_BottomUp prefix_sum_BottomUp;
        ...
        for (int i = 0; i < m+k; i++) {
            ...
            switch (option) {
                case 1:
                    ...
                    prefix_sum_TopDown.update(1, 0, n-1, target-1, diff); // x가 아니라 dx! - 트리의 값 변경 (Top-Down)
                    break;
                case 2:
                    int last = Integer.parseInt(st.nextToken());
                    sb.append(prefix_sum_TopDown.sum(1, 0, n-1, target-1, last-1)).append('\n');
                    break;
            }
        }
    }
        ...
        for (int i = 0; i < m+k; i++) {
            ...
            switch (option) {
                case 1:
                    ...
                    arr[target-1] = change;     // *원본 배열에도 변한값 업데이트
                    prefix_sum_BottomUp.update(target-1, change);
                    break;
                case 2:
                    int last = Integer.parseInt(st.nextToken());
                    sb.append(prefix_sum_BottomUp.sum(target-1, last-1)).append('\n');
                    break;
            }
        }
    }
