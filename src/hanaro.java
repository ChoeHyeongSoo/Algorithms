...
class Solution {
    public static void main(String args[]) throws Exception {
        ...
        for (int test_case = 1; test_case <= T; test_case++) {
            ...
            for (int i = 0; i < edges.size(); i++) {
                ...
                connected++; cost+=(ratio * Math.pow(curr.d, 2));
    ...
    public static boolean union(int x, int y) {
        ...
        if (root_x < root_y) parents[y] = root_x;
        else parents[x] = root_y;
    ...
    public static double cal_distance(Island u, Island v) {

        int x = Math.abs(u.x - v.x), y = Math.abs(u.y- v.y);
